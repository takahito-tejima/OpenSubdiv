                   ...
                   int array)
{
    ...
    for (int i = 0; i < numPatches; i++) {
        const int *verts = &patchTable->GetPatchArrayVertices(array)[i*16];
        ...
        OpenSubdiv::Far::PatchParam param = patchTable->GetPatchParams(array)[i];
        int boundaryMask = param.bitField.GetBoundary();
        ...
        cpIndices.push_back(patchTable->GetPatchArrayVertices(array)[i*16 + 5]);
        cpIndices.push_back(patchTable->GetPatchArrayVertices(array)[i*16 + 6]);
        cpIndices.push_back(patchTable->GetPatchArrayVertices(array)[i*16 + 9]);
        cpIndices.push_back(patchTable->GetPatchArrayVertices(array)[i*16 + 10]);
    }
    return numPatches;
}
